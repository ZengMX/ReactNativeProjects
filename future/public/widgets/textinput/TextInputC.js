...
import ReactNative, { StyleSheet, TextInput, View, Platform, TouchableOpacity, Image,InteractionManager } from 'react-native';
...
class _TextInput extends Component {
	...
	firstIn() {
		if (!this.isFirstIn&&this.props.autoFocus) {
			this.isFirstIn = true;
			InteractionManager.runAfterInteractions(() => {
				TextInputState.focusTextInput(ReactNative.findNodeHandle(this.refs.input));
			})
		}
	}

	setTxtNativeProps(value){
	...
	_renderClearBtn() {		
		if (Platform.OS != 'android' || this.props.noClean) {
			return null;
		}
		/*const cleanBtn = (
		...
		let cleanBtn = (
			<TouchableOpacity onPress={this._clearInput.bind(this)}
				hitSlop={{ top: 5, left: 5, bottom: 5, right: 5 }}
				style={{ position: 'absolute', right: 8, top: (this.propsStyle.height - 15) / 2, backgroundColor: 'transparent' }}
				>
				<Image source={require('./res/008shurucha.png')}
	...
	_renderInput() {
		let { style, onChangeText, autoFocus, ...inputProps } = this.props;
