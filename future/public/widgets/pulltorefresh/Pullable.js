...
const defaultTopIndicatorHeight = defaultHeight; //顶部刷新指示器的高度
    ...
    render() {
        ...
        if (this.props.topIndicatorRender == null) {
            topIndicator = (
                ...
                height: defaultTopIndicatorHeight,width : screenWidth}}>
                    <ActivityIndicator size="small" color="gray" />
                    {this.state.pulling ? <Text style={{marginLeft:10}}>下拉刷新</Text> : null}
                    {this.state.pullok ? <Text style={{marginLeft:10}}>松开刷新</Text> : null}
                    {this.state.pullrelease ? <Text style={{marginLeft:10}}>加载中</Text> : null}
                </View>
            );
        } else {
        ...
        return (
            <View style={[{flex: 1,flexDirection: 'column',zIndex:-999}, this.props.style]} onLayout={this.onLayout}>
