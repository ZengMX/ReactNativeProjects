 ...
 *
 * Props:
 *	autoRefresh 	: false  // 是否进入页面就加载, 默认false
...
class RefreshableListView extends Component {
	...
	_onLoadMore(){		
		if(this.state.status !== 'loading' && this.state.isRefreshing == false && this.state.status != 'error'){			
			this.setState({status : 'loading'});			
			//page自加放在这里可以解决列表连续快速刷新出现重复数据的bug
			this.page++;
			// if(this.totalRowCount!=null&&this.totalCount!=null&&this.totalRowCount >= this.totalCount) {
				this._onRefresh();
			// } else {
			// 	this.reloadData();
			// 	this.setState({
	...
	_onRefresh() {
		this.props.fetchData(this.page + 1, (rows, currCount, totalCount) => {
			...
			this.totalCount = totalCount;
			// InteractionManager.runAfterInteractions(() => {
			// this.refs.listview.getNativeListView().scrollTo&&this.refs.listview.getNativeListView().scrollTo({x: 0, y: 0});
			// })
	...
	setRows(rows){
		//console.log('setRows', this.totalRowCount >= this.totalCount ? 'done' : 'wait')
		this.setState({
			//status : this.totalRowCount >= this.totalCount ? 'done' : 'wait',
			isRefreshing : false,
	...
	_renderFooter(){
		...
		if(this.state.isMyriad==true && this.props.customEmpty){
			return this.props.customEmpty;
		}else if(this.state.isMyriad==true){
			return (<View>
	...
	propsRenderFooter(){
		return(
			<View style={[{marginVertical:20}, this.props.renderFooterStyle]}>
				{
