...
class FillContactorInfos extends Component {
    constructor(props) {
        super(props);
    ...
    _initData() {
        if (this.props.params.buyManager != undefined) {
            ...
            console.log('buyManager', buyManager);
    ...
    _isPerfect() {
        if (!ValidateUtil.isNull(this.state.buyManagerNm)
            ...
            && !ValidateUtil.isNull(this.state.receiveZoneIdStr)
            && !ValidateUtil.isNull(this.state.receiveManagerAddr)) {
            this.full = true;
        } else {
            this.full = false;
        }
    ...
    _saveContactorInfos() {
        this._isPerfect();
        if (!(ValidateUtil.isNull(this.state.buyManagerMobile)) && this.state.buyManagerMobile.length != 11) {
            Toast.show('请填写正确的采购人联系电话');
            return;
        }
        if (!(ValidateUtil.isNull(this.state.receiveManagerMobile)) && this.state.receiveManagerMobile.length != 11) {
            Toast.show('请填写正确的收货人联系电话');
            return;
        }
        new Fetch({
        ...
        this.refs.base.pickAreaParams(null, "http://eyb2b.imall.com.cn", (areaCode, addressString) => {
            if (!ValidateUtil.isBlank(areaCode) && !ValidateUtil.isBlank(addressString)) {
                if (Platform.OS == 'ios') {
                    ...
                    this.setState({
                        receiveZoneIdStr: address.trim().replace(/\s/g, "-")
                    });
                } else {
                    this.setState({
                        receiveZoneIdStr: addressString
                    });
                }
                this.setState({
                    ...
                    <View style={{ flex: 1, flexDirection: 'column' }}>
                        ...
                        <TouchableOpacity style={styles.zoneIdPress} onPress={this.onBuyManagerZoneIdPress.bind(this)}>
                            ...
                            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' }}>
                                <Text numberOfLines={1} style={{ color: '#AAAEB9', fontSize: 14, marginRight: 8 }}>{this.state.receiveZoneIdStr}</Text>
...
export default connect(mapStateToProps)(FillContactorInfos);