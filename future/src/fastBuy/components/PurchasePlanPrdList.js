...
import ShoppingPlan from '../../shoppingCart/components/ShoppingPlan';

import styles from '../styles/PurchasePlanPrdList';

var fullWidth = require('Dimensions').get('window').width;
var fullHeight = require('Dimensions').get('window').height;
class PurchasePlanPrdList extends Component{
	constructor(props){
		super(props);
		this.state = {
			num : 0,			//采购计划商品数量
		...
		this.purchaseTemplateId = this.props.params.purchaseTemplateId || 1;	//模板Id
		this.templateNm = "采购计划";	//采购计划名称
		this.editNumArr = {};			//记录编辑状态下(常购无该属性) 修改过数量的商品skuId 与 修改后数量的商品buyNum
	}

	componentWillUnmount(){
		//处理采购计划列表回调(不包括速购入口，仅会员中心进入可编辑)
		if(this.props.params){
			let rows = [].concat(this.refs.list.getRows());
			let kindNum = this.canEditLength;
			let count = 0;
			...
			this.props.params.resetPlanCount && this.props.params.resetPlanCount(kindNum, count, index);//参数左至右：种类、总数、位置
		}
	}
	...
	onDelete(){
		...
		let skuIds = this.returnParam(selectedEditArr);
		Alert.alert('温馨提醒', '确定要删除所选的商品?', [
			{ text: '取消', onPress: () => { } },
			{
				text: '确定', onPress: () => {
					Loading.show();
					new Fetch ({
						url : "/app/user/delPurchaseTemplateItem.json",
						method : "POST",
						data : {
							templateId : this.purchaseTemplateId,
							skuIds : skuIds
						},
					}).dofetch().then((data)=>{
						Loading.hide();
	...
	addToPurchase(){
		...
		Loading.show();
		new Fetch ({
			url : "/app/cart/addPurchases.json",
			method : "POST",
			data : {
				skuIds : skuIds
			},
	 ...
	 */
	editPurchasePlan(){
		console.log(this.editNumArr);
		let skuIdArr = _.keys(this.editNumArr);
		let buyNumArr = _.values(this.editNumArr);
		if (skuIdArr.length <= 0 || buyNumArr.length <= 0) {
			return;
		}
		let skuIds = this.returnParam(skuIdArr);
		let buyNums = this.returnParam(buyNumArr);
		Loading.show();
		new Fetch ({
			url : "/app/cart/addPurchaseTemplateList.json",
			method : "POST",
			data : {
				purchaseTemplateId : this.purchaseTemplateId,
	 ...
	 */
	goAddPurchasePlan(){
		let skuIdArr = _.keys(this.state.selectedEditMap);
		let buyNumArr = _.values(this.state.selectedEditMap);
		if (skuIdArr.length <= 0 || buyNumArr.length <= 0) {
			Toast.show("请选择至少一个商品");
			return;
		}
	...
	onSelectAll(value) {
		...
		if(this.state.isEdit){
			_.map(rows, (item,index)=>{
				...
				rows[index] = row;
				value && (this.state.selectedEditMap[rows[index].skuId] = rows[index].buyNum);
			})
	...
	onSelect(data) {
		if(this.state.isEdit){
			...
			if (data.isSelected) {
				this.state.selectedEditMap[data.skuId] = data.buyNum;
				if(_.size(this.state.selectedEditMap) == this.canEditLength){
					isSelectAll = true;
				}
			} else {
				delete this.state.selectedEditMap[data.skuId];
			}
			this.setState({
				selectedEditMap : this.state.selectedEditMap,
	...
	/**
	 * 注：此处与常购相比多了skuId，主要用于保存数据（skuId，buyNum）供编辑采购计划数量使用
	 * 商品数量或勾选状态改变时更新底部总计及selectedNorMap
	 * @param changeValue		//价格变动
	 * @param selectedNorMap	
	 * @param prdTotalPrice		//购买商品小计
	 * @param itemIndex			//购买商品位置（列表中）
	 * @param buyNum			//购买商品数量
	 * @param skuId				//购买商品skuId
	 */
	onBuyNumChange(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId){
		this.editNumArr[skuId] = buyNum;
		//更新列表数据
	...
	/**
	 * 设置初始总计、可选商品数量、可编辑商品数量、已选商品Map（普通状态）、可编辑商品Map（编辑状态）、模板名称
	 * @param count
	 ...
	 * @param templateNm
	 */
	setMainData(count, canSelectLength, canEditLength, selectedNorMap, selectedEditMap, templateNm){
			...
			<View style={styles.navbarContainer}>
				{/* 编辑 */}
				{
					...
					onPress={()=>{
						...
						this.refs.list.setRows(rows);
						this.state.isEdit && this.editPurchasePlan();
						this.setState({
				...
				</TouchableOpacity>
				{
					...
					<View style={styles.editBottomView}>
						<TouchableOpacity style={styles.delBtn}
							...
							<Image style={styles.delBtnImg} source={require('../res/purchasePrdList/065jiachanggou.png')}></Image>
							<Text style={styles.delBtnText}>加常购</Text>
						</TouchableOpacity>
						<TouchableOpacity style={styles.delBtn}
							activeOpacity={0.7}
							onPress={()=>{
								this.goAddPurchasePlan();
							}}>
							<Image style={styles.delBtnImg} source={require('../res/purchasePrdList/065jiajihua.png')}></Image>
							<Text style={styles.delBtnText}>加计划</Text>
						</TouchableOpacity>
						<TouchableOpacity style={styles.delBtn}
							activeOpacity={0.7}
							onPress={()=>{
	...
	render(){
		let title = this.state.num > 0 ? (this.templateNm + "(" + this.state.num + ")") : this.templateNm;
		return (
			<BaseView navigator={this.props.navigator}
				ref='base'
				paddingRight={this.props.params.isFast ? 0 : 60}
				...
				{!this.canEditLength && <DataController />}
					<View style={styles.container}>
						...
						purchaseTemplateId={this.purchaseTemplateId}
						setMainData={(count, canSelectLength, canEditLength, selectedNorMap, selectedEditMap, templateNm)=>{
							this.setMainData(count, canSelectLength, canEditLength, selectedNorMap, selectedEditMap, templateNm)
						}}
						...
						onBuyNumChange={(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId)=>{
							this.onBuyNumChange(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId)
						}}/>
					...
					<View style={{overflow:"hidden", height: this.state.type === 2 ? fullHeight : 0 , width: fullWidth, backgroundColor: "#f2f4f5", alignItems:"center", position: "absolute", top: 0 }}>
						...
						<Text style={{fontSize: 14, color: "#999", marginTop: 10, marginBottom: 10}}>该采购计划还没有商品哦!</Text>
						{
							this.props.params.isFast && <TouchableOpacity
					...
					<MaskModal
						...
						contentView={<ShoppingPopup product={this.state.modalProduct}
							...
							selectedNorMap={this.state.selectedNorMap}
							onBuyNumChange={(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId)=>{this.onBuyNumChange(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId)}}
...
class List extends Component{
	...
	_fetchData(page, success, error){
		...
		InteractionManager.runAfterInteractions(() => {
			//动画执行完毕再执行
			new Fetch ({
				url : "/app/fastBuy/purchasePlanPrdList.json",
				mothod : 'POST',
				data: {
					purchaseTemplateId : this.props.purchaseTemplateId,
				}
			}).dofetch().then((data)=>{
				...
				for(let i = 0; i < result.length; i++){
					if((result[i].isOutOfStock != "Y" && result[i].isOnSale != "N" && result[i].isBannedProduct != "Y")){
						...
						result[i].buyNum = (result[i].num || 0);
						...
						result[i].buyNum = result[i].num;
						result[i].totalPrice = result[i].price * (result[i].num || 0);
					}
				}
				this.props.setMainData(count, canSelectLength, result.length, this.props.selectedNorMap, {}, data.templateNm);
	...
	_renderRow(rowData, sectionID, rowID){
		return (
			...
			onBuyNumChange={(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId)=>{
				this.props.onBuyNumChange(changeValue, selectedNorMap, prdTotalPrice, itemIndex, buyNum, skuId);
			}}
...
class PrdItem extends Component{
	...
	_onChange(buyNum){
		...
		this.timer = setTimeout(()=>{
			...
			this.props.onBuyNumChange && this.props.onBuyNumChange(changeValue, selectedNorMap, num > 0 ? (num * this.props.prdPrice) : 0, this.props.itemIndex, num, this.props.skuId);
		},200); 
	}
					...
					isSelected && <View style={styles.bottomView}>
						<View style={{flex: 1, height: 55, backgroundColor: "#fafafa", paddingHorizontal: 10, flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
							<View style={{height: 55, justifyContent: "space-between", paddingVertical: 8}}>
								<Text style={{marginRight: 8}} numberOfLines={1}
								text={[
								{ value: this.props.isBannedProduct ? "" : "￥", style: {fontSize: 10, color: "#53606a"} },
								{ value: this.props.isBannedProduct ? "该商品禁销" : (this.props.prdPrice).toFixed(2), style: {fontSize: 13, color: "#53606a"} },
								]}></Text>
								<Text style={{fontSize: 12, color: "#323232"}} numberOfLines={1}>库存:{this.props.prdRemainStockRange}</Text>
							</View>
							<View style={{alignItems:"center"}}>
						...
						<View style={{flex: 1, height: 40, marginTop: 15, borderTopWidth: 1/PixelRatio.get(), borderTopColor: "#e5e5e5", flexDirection: "row", justifyContent: "space-between", alignItems: "center"}}>
							...
							<Text style={{marginRight: 8}} numberOfLines={1}
							text={[
							{ value: this.props.isBannedProduct ? "" : "小计:￥", style: {fontSize: 10, color: "#53606a"} },
							{ value: this.props.isBannedProduct ? "该商品禁销" : (this.props.prdTotalPrice).toFixed(2), style: {fontSize: 14, color: "#53606a"} },
							]}></Text>
						</View>
					</View>
				}
...
export default connect(mapStateToProps, mapDispatchToProps)(PurchasePlanPrdList);